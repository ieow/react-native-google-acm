...
export function testCrypto(hashInput, loop, Crypto) {
  ...
  let hash = crypto.createHash('sha256').update(hashInput).digest('hex');
  for (let i = 0; i < loop; i++) {
    const _hash = crypto.createHash('sha256').update(hash).digest('hex');
    hash = _hash;
  }
  console.log('=============hash time==============', performance.now() - start);
  return hash;
}
export function testEncryption(loop, Crypto) {
  let start = global.performance.now();
  let crypto = Crypto || global.crypto;
  ...
  console.log('=============encryption time==============', performance.now() - start);
  return out;
}
